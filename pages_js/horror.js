...
window.addEventListener('DOMContentLoaded', function(){
  ...
  video.addEventListener('click', function(){
    ...
    video.insertAdjacentHTML('afterbegin', '    <iframe  src="https://www.youtube.com/embed/qvWEra3kja4" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>    ')
...
import { arrImgFourth } from "../scriptPicture";
...
`;
  ...
  fourthGroup.append(div);
}


function createlistUser(arr) {
  fourthGroup.innerHTML = "";
  arr.forEach((item) => {
    createUser(item);
  });
}
...
async function getUsers() {
  const responce = await fetch("http://localhost:3003/fourth_group_movies");
  const data = await responce.json();
  const newData = data.map((item,index)=>{
    item.img = arrImgFourth[index]
    return item
  })
...
async function search() {
  ...
  if (value) {
    ...
    if (newArr.length > 0) {
      ...
      fourthGroup.innerHTML = `<h1 >Ничего не найдено</h1>`
    }
