...
const secondGroup = document.querySelector(".cartoons_group_movies")

import { arrImgSecond } from "../scriptPicture";
...
async function getUsers() {
  const responce = await fetch("http://localhost:3003/cartoons_group_movies");
  const data = await responce.json();
  const newData = data.map((item,index)=>{
    item.img = arrImgSecond[index]
    return item
  })
